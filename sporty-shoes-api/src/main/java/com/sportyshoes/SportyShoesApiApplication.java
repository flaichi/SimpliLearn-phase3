...
public class SportyShoesApiApplication {
	...
	CommandLineRunner start(ShoesRepository repo) {
		return args ->{
			Shoe shoe = new Shoe();
			shoe.setBrand(BrandEnum.NIKE);
			shoe.setColor(ColorEnum.RED);
			shoe.setDescription("great running shoes for girls");
			shoe.setGender(GenderEnum.GIRLS);
			shoe.setName("Nike gtr2");
			shoe.setSize(SizeEnum.Three_five);
	...
	// uncomment this to put a customer in the database
		...
		CommandLineRunner start(CustomerRepository repo) {
			return arg ->{
				...
				customer.setName("a given customer");
	...
	// uncomment this to put users and and admin in the database
			...
			CommandLineRunner start(AccountService service) {
				return arg ->{
					service.saveRole("USER_READ");
					service.saveRole("USER_WRITE");
					service.saveRole("ADMIN_READ");
					service.saveRole("ADMIN_WRITE");
					service.saveUser("user1", "Upass2", "Upass2");
					service.saveAdmin("admin1", "Apass1", "Apass1");
